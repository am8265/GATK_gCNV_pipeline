...
shard_ct="$3"
model_shard=$(seq 1 ${shard_ct} | while read -r shard; do echo --model-shard-path /nfs/tx/in/CNV_WES/GATK-gCNV/tx-6278_IC/IDTERPv2/00${shard}/IDTERPv2100Cohort-model; done)
calls_shard=$(seq 1 ${shard_ct} | while read -r shard; do echo --calls-shard-path /nfs/tx/in/CNV_WES/GATK-gCNV/tx-6278_IC/IDTERPv2/00${shard}/IDTERPv2100Cohort-calls; done) 
